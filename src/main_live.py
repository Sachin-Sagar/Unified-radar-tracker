...
class RadarWorker(QObject):
    ...
    def run(self):
        ...
        if not self.params_radar or not self.h_data_port:
            print("Failed to configure sensor. Exiting worker thread.")
            self.stop()
            self.finished.emit()
            return
        ...
        while self.is_running:
            ...
            print(f"Frame: {self.tracker.frame_idx} | Detections: {frame_data.num_points} | Confirmed Tracks: {num_confirmed_tracks}")
    ...
    def _save_tracking_history(self):
        ...
        if self.tracker and self.fhist_history:
            ...
            print("No frame history was processed, nothing to save.")
